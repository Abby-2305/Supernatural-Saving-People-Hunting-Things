    ...
    while not gameExit:
        ...
        for eachEnemy in EnemyGroup:
            ...
            if eachEnemy.rect.x<0:
                ...
                EnemyCount+=1
            if eachEnemy.rect.centerx == Player.rect.centerx:
                score +=10
                Player.health-=10
            if Enemyhits:
                Player.health= 0
...
button = PushButton(ButtonBox, text ="Play", width = "fill", command=playGame)
button.text_size = int(MenuHeight/30)
button.text_color = DarkRed2

button.when_right_button_pressed = rightclicked
